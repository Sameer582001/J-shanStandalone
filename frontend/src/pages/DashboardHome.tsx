    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-accent-cyan flex items-center gap-2">
                        {activeNode ? `Node Dashboard: ${activeNode.referralCode}` : 'Master Dashboard'}
                        <button
                            onClick={handleManualRefresh}
                            className={`p-1.5 rounded-full hover:bg-dark-surface transition ${loading ? 'animate-spin' : ''}`}
                            title="Refresh Data"
                        >
                            <RefreshCw size={18} className="text-accent-teal" />
                        </button>
                    </h2>
                    {activeNode && <span className="text-sm text-gray-400">Viewing performance for specific node</span>}
                </div>

                {activeNode ? (
                    <button
                        onClick={clearActiveNode}
                        className="bg-dark-surface text-gray-300 border border-gray-700 px-4 py-2 rounded hover:bg-gray-800 transition"
                    >
                        Exit Node View
                    </button>
                ) : (
                    <Link
                        to="/purchase-node"
                        className="bg-accent-teal text-white px-4 py-2 rounded hover:bg-teal-700 transition shadow-sm border border-transparent"
                    >
                        Purchase New Node
                    </Link>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-dark-surface p-6 rounded-lg shadow-sm border border-gray-800">
                    ...
                    {!activeNode && (
                        <p className="text-sm text-gray-400 mt-1">Total Earnings: ₹{masterStats?.totalEarnings || '0.00'}</p>
                    )}
                </div>
            ...
            {!activeNode && (
                <div className="mt-8 bg-dark-surface p-6 rounded-lg shadow-sm border border-gray-800 flex flex-col md:flex-row items-center justify-between">
                    <div>
                        <h3 className="text-lg font-bold text-white mb-1">Manage Your Nodes</h3>
                        <p className="text-gray-400">View performance details and login to specific nodes.</p>
                    </div>
                    <Link
                        to="/my-nodes"
                        className="mt-4 md:mt-0 bg-accent-teal text-white px-6 py-2 rounded-lg font-semibold hover:bg-teal-700 transition shadow-sm"
                    >
                        Go to My Nodes
                    </Link>
                </div>
            ...
            <div className="mt-8 bg-dark-surface p-6 rounded-lg shadow-sm border border-gray-800">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-medium text-white">Recent Activity</h3>
                    <Link to="/wallet/transactions" className="text-sm text-accent-cyan hover:text-cyan-300">
                        View All
                    </Link>
                </div>
                    ...
                    <div className="space-y-4">
                        {transactions.map((tx) => (
                            <div key={tx.id} className="flex justify-between items-center border-b border-gray-700 pb-2 last:border-0 last:pb-0">
                                <div>
                                    <p className="font-semibold text-gray-200">{tx.description}</p>
