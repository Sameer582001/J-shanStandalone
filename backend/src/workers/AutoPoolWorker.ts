...
// const financialService = new FinancialService(); // Remove global instance causing race conditions
...
export const initAutoPoolWorker = () => {
    const worker = new Worker('auto-pool-queue', async job => {
        ...
        try {
            ...
            // 3. Process Financials (AUTO POOL INCOME + Multi-Level)
            const financialService = new FinancialService();
            // Old: await financialService.processIncome(parentId, AUTO_FEE, 1, 'AUTO', client);
            // New: Distribute to all eligible ancestors
            await financialService.distributeNewNodeCommissions(nodeId, 'AUTO', client);

            await client.query('COMMIT');

            // 4. Trigger Secondary Auto Pool Placements (Rebirths generated by Income)
            if (financialService.createdRebirthIds.length > 0) {
                const queue = new Queue('auto-pool-queue', {
                    connection: {
                ...
                console.log(`[AutoPoolWorker] Queueing ${financialService.createdRebirthIds.length} Secondary Rebirths...`);
                for (const rebirthId of financialService.createdRebirthIds) {
                    await queue.add('NEW_REGISTRATION', { nodeId: rebirthId });
                }
                await queue.close();
            }
